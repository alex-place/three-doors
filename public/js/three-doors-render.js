...
function appendSceneMsg(sceneKey, sceneData, geminiText, source) {
  ...
  const castStrip = sceneKey === "castle-balcony" && !sceneData.resumed
    ? `<div class="cast-strip" aria-label="The companions, as Alex draws them">
        <figure><img src="/assets/content/koh/reference-lantern-t.webp" alt="Lantern" onerror="this.closest('figure').style.display='none'"><figcaption>Lantern</figcaption></figure>
        <figure><img src="/assets/content/koh/reference-eclipse-t.webp" alt="Eclipse" onerror="this.closest('figure').style.display='none'"><figcaption>Eclipse</figcaption></figure>
        <figure><img src="/assets/content/koh/reference-keystone-t.webp" alt="Keystone" onerror="this.closest('figure').style.display='none'"><figcaption>Keystone</figcaption></figure>
        <figure><img src="/assets/content/koh/reference-blinkbug-t.webp" alt="Blinkbug" onerror="this.closest('figure').style.display='none'"><figcaption>Blinkbug</figcaption></figure>
      </div>`
    : "";
...
async function enrichScene(sceneKey, textEl, msgEl) {
  if (!textEl) return;
  if (typeof serverAvailable !== "undefined" && !serverAvailable) return;
  if (typeof narratorEnabled !== "undefined" && !narratorEnabled) return;
  const scene = SCENES[sceneKey];
  if (!scene) return;
  ...
  // Image-grounded beat from the game's own narrator. The engine's canon doors
  // (and the player's name-your-own-door input) carry the freeform choices — we
  // enrich the PROSE to describe this exact painting, not swap the doors.
  const ctrl = new AbortController();
  setTimeout(() => ctrl.abort(), 25000);
  try {
    const r = await fetch("/api/scene/narrate", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        sceneKey,
        sceneText: (scene.text || "").replace(/\*/g, "").slice(0, 300),
        imageDesc: visionDesc || (art ? art.prompt : ""),
        theme, lesson,
        choice: recentChoices,
        playerWords: desires,
    ...
    if (!r.ok) return;
    const data = await r.json();
    const narration = (data.reply || "").trim();
