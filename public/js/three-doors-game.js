...
async function apiNarrate(sceneKey, sceneText) {
  // The beat is painted fresh each turn by the game's own narrator (Gemini on
  // Vertex, /api/scene/narrate) — the cast, the creed, the grown-up dream tone.
  // Grounded in this scene + the door just chosen + every canon the player has
  // authored (their inventions outrank ours).
  ...
  try {
    const r = await fetch("/api/scene/narrate", {
      ...
      body: JSON.stringify({
        sceneKey,
        sceneText: (sceneText || scene.text || "").replace(/\*/g, "").slice(0, 400),
        theme: scene.theme || "",
        lesson: scene.lesson || "",
        choice: gameState?.last_choice || "",
        playerWords: (playerProgress?.customDesires || []).slice(-1)[0] || "",
...
async function getSceneData(action, label) {
  ...
  // The engine's canon text shows instantly; enrichScene() then replaces it
  // with an image-grounded beat once the painting resolves — one narrator, and
  // the scene never waits on the model to first appear.
  return { data, geminiText: "", source: "engine" };
}
